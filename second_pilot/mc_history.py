...
for stat in stats:
	...
	stat_by_user[user_id]["d"] = items_col.count({"userId": user_id, "kindDifficulties.MultipleChoice" : {"$gt":0.5}})
	
stat_by_user = {user_id: stat_by_user[user_id] for user_id in stat_by_user if stat_by_user[user_id]["d"] > 100}
stat_by_user = {user_id: stat_by_user[user_id] for user_id in stat_by_user if stat_by_user[user_id]["a"] > 5}

for user_id in stat_by_user:
	board_field = user_id + '_Board'
	user_games = games_col.find({board_field: {'$exists' : True}}).sort([("creationTime", -1)]).limit(5)
	cnt = 0
	for game in user_games:
		board = game[board_field]
		for tile in board["tiles"]:
			if tile["question1"]["kind"] == "MultipleChoice":
				stat_by_user[user_id]["mc"]["a"] += 1
				if "correct" in tile["question1"] and tile["question1"]["correct"]:
					stat_by_user[user_id]["mc"]["c"] += 1
...
res_tl = [(stat_by_user[user_id]["mc"]["c"]/stat_by_user[user_id]["mc"]["a"]) * 100 for user_id in stat_by_user if stat_by_user[user_id]["mc"]["a"] > 0]
...
plt.title('Multiple Choice questions games played/success rate scatter plot')
